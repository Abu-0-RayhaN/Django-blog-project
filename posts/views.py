...
# this is a custom filter for showing replies on comment. but i have figured out another easy way so that's why not using this way
...
def Creating_blog(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():  # updated on recommandation of chatgpt
...
def bloglist(request):
    ...
    if request.method == "GET":
        ...
        if not search == ' ':
            # i am using this contains key to get related name user.
